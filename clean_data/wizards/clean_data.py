...
class CleanData(models.TransientModel):
    ...
    def clean_base_data(self,):
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: mail_activity")

        try:
            sql = """delete from res_users where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: res_users")
        ...
        #     self._cr.execute(sql)
        # except:
        #     _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: res_partner")
        ...
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_partial_reconcile")
        ...
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_move_line")
        ...
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_move")
    ...
    def check_and_delete(self,table):
        ...
        if table == 'stock_quant':
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_quant")
            ...
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_move_line")
            ...
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_move")
            ...
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_picking_type")
            ...
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_picking")
        else:
            # sql = """SELECT EXISTS (
            ...
            try:
                ...
                _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: %s" % table)
    ...
    def _clear_cus_ven(self):
        ...
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_fiscal_position")

        try:
            sql = """delete from purchase_requisition where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: purchase_requisition")

        try:
            sql = """delete from stock_rule where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_rule")
        ...
        try:
            ...
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_valuation_layer")

        try:
            sql = """delete from stock_warehouse where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: stock_warehouse")

        try:
            sql = """delete from account_followup_followup_line where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_followup_followup_line")

        try:
            sql = """delete from account_reconcile_model where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: account_reconcile_model")

        try:
            sql = """delete from ir_property where company_id=%s""" % self.company_id.id
            self._cr.execute(sql)
        except:
            _logger.info("\n XXXXXXXXXXXXXXXX No se pudo borrar la tabla: ir_property")

        ########## Borrando todo a pata ###################
        self.env.cr.execute("""update product_template set website_id=null where website_id in (select id from website where company_id%s);
        delete from product_public_category where website_id in (select id from website where company_id%s);
        delete from payment_acquirer where company_id%s;
        delete from website where company_id%s;
        delete from res_users where company_id%s;
        delete from account_partial_reconcile 
                        where debit_move_id in (select id from account_move_line 
                            where move_id in (select account_move.id from account_move where company_id%s ));
        ...
        delete from stock_quant where company_id%s;
        delete from stock_move_line where company_id%s;
        delete from stock_move where company_id%s;
        delete from stock_picking where company_id%s;
        delete from account_partial_reconcile where company_id%s;
        delete from account_payment_register where company_id%s;
        delete from account_move_line where company_id%s;
        delete from account_move where company_id%s;
        delete from sale_order_line where company_id%s;
        delete from sale_order where company_id%s;

        delete from account_payment where id in (select account_payment.id from account_payment join account_move on account_move.id = account_payment.move_id where account_move.company_id%s);
        ...
        delete from account_transfer_model_line where id in (select account_transfer_model_line.id from account_transfer_model_line 
                                             ...
                                             join account_journal 
                                               on account_journal.id = account_transfer_model.journal_id
                                      where account_journal.company_id%s); 

        delete from account_transfer_model where id in (select account_transfer_model.id from account_transfer_model 
                                             join account_journal 
                                               on account_journal.id = account_transfer_model.journal_id
                                      where account_journal.company_id%s); """, (self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,
                                                                                 self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,
                                                                                 self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id,self.company_id.id
                                                                                ))
